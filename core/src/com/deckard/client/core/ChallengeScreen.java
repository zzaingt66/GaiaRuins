...
/**
 * Pantalla de desafío con carta de decisión
 */
public class ChallengeScreen implements Screen {
    ...
    public ChallengeScreen(GameScreen game, GaiaRuinsGame gaiaGame) {
        this.game = game;
    ...
    private void createUI() {
        ...
        DecisionCard card = gaiaGame.getCurrentCard();
        ...
        // Titulo
        Label titleLabel = new Label("DESAFIO AMBIENTAL", skin);
        titleLabel.setFontScale(2f);
        titleLabel.setColor(Color.ORANGE);

        // Categoria
        Label categoryLabel = new Label(card.getCategory().toString(), skin);
        categoryLabel.setFontScale(1.3f);
        categoryLabel.setColor(Color.YELLOW);

        // Titulo de la carta
        Label cardTitleLabel = new Label(card.getTitle(), skin);
        cardTitleLabel.setFontScale(1.5f);
        cardTitleLabel.setColor(Color.WHITE);

        // Pregunta verdadero/falso
        Label questionLabel = new Label(card.getQuestion(), skin);
        questionLabel.setFontScale(1.2f);
        questionLabel.setWrap(true);

        // Barras de vida: Jugador vs Contaminacion
        Table healthBarsTable = createHealthBars();

        mainTable.add(titleLabel).padBottom(10);
        mainTable.row();
        mainTable.add(categoryLabel).padBottom(15);
        mainTable.row();
        mainTable.add(cardTitleLabel).padBottom(15);
        mainTable.row();
        mainTable.add(questionLabel).width(800).padBottom(30);
        mainTable.row();
        mainTable.add(healthBarsTable).width(900).padBottom(30);
        mainTable.row();

        // Botones Verdadero y Falso
        Table optionsTable = new Table();
        for (int i = 0; i < card.getOptions().size(); i++) {
            DecisionOption option = card.getOptions().get(i);
            TextButton optionButton = createTrueFalseButton(option, i);
    ...
    private TextButton createTrueFalseButton(DecisionOption option, int index) {
    ...
    private void selectOption(int index, DecisionOption option) {
        ...
        DecisionCard card = gaiaGame.getCurrentCard();
        boolean isCorrect = card.isCorrectAnswer(option.getText().equals("VERDADERO"));

        if (isCorrect) {
            resultText = "CORRECTO! " + card.getExplanation();
        } else {
            resultText = "INCORRECTO. " + card.getExplanation();
        }
    ...
    private void showResult() {
        ...
        Label resultTitleLabel = new Label("RESULTADO", skin);
        resultTitleLabel.setFontScale(2f);
        resultTitleLabel.setColor(Color.GREEN);

        Label resultTextLabel = new Label(resultText, skin);
        resultTextLabel.setFontScale(1.3f);
        resultTextLabel.setWrap(true);
        ...
        resultTable.add(resultTitleLabel).padBottom(30);
        resultTable.row();
        resultTable.add(resultTextLabel).width(800).padBottom(30);
        resultTable.row();
    ...
    public void render(float delta) {
        ScreenUtils.clear(0.15f, 0.1f, 0.1f, 1);
